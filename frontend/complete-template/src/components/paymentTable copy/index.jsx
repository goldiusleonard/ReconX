...
function ConsolidatedTable({ data }) {
      ...
      {data.map((dataItem, index) => (
        <div key={index}>
          {Object.keys(dataItem).map((category) => (
            <ConsolidatedTableSection key={`${index}-${category}`} title={formatTitle(category)} transactionsData={dataItem[category]} />
          ))}
        </div>
      ))}
    </Card>
...
function ConsolidatedTableSection({ title, transactionsData }) {
